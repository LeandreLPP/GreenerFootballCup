...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        admin_code.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (!confirm_admin_code.getText().toString()
                                       .equals(admin_code.getText().toString()))
                {
                    LoginDatas.getInstance().setAdminCode(null);
                    confirm_admin_code
                            .setError(getResources().getString(R.string.passwords_do_not_match));
        ...
        admin_email.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (!confirm_admin_email.getText().toString()
                                        .equals(admin_email.getText().toString()))
                {
                    LoginDatas.getInstance().setEmailAddress(null);
                    confirm_admin_email
                            .setError(getResources().getString(R.string.email_do_not_match));
                    if (btnToScreen3.isEnabled())
                        btnToScreen3.setEnabled(false);
                }
                else
                {
                    LoginDatas.getInstance()
                              .setEmailAddress(confirm_admin_email.getText().toString());
